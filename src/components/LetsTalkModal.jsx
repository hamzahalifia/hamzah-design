...
export default function LetsTalkModal({ isOpen, onClose }) {
  useEffect(() => {
    if (isOpen) {
      ...
      // Fix for Cal.com embed accessibility
      const intervalId = setInterval(() => {
        const calEmbed = document.querySelector('.cal-inline-container');
        if (calEmbed) {
          calEmbed.setAttribute('tabindex', '0');
      ...
      {isOpen && (
        <div className="fixed inset-0 z-[9999] flex items-end sm:items-center justify-center p-0 sm:p-6">
          ...
          <div className="relative z-10 w-full sm:w-[500px] mx-auto">
           ...
           {/* ── 1. Title Section ── */}
              <div className="relative z-10 p-5 border-b border-attio-border-light dark:border-attio-border-dark space-y-3">
                {/* Profile photo row */}
              ...
              <div className="grid grid-cols-2 divide-x divide-attio-border-light dark:divide-attio-border-dark border-b border-attio-border-light dark:border-attio-border-dark">
                {/* WhatsApp */}
                <a
              ...
              <a
                ...
                rel="noopener noreferrer"
                onClick={onClose}
                className="group py-6 px-4 flex flex-row items-center justify-between border-b border-attio-border-light dark:border-attio-border-dark hover:bg-neutral-50 dark:hover:bg-neutral-900/50 transition-colors cursor-pointer"
              >
