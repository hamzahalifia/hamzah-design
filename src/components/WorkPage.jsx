...
import { cmsFetch, CASE_STUDIES_QUERY } from "../lib/cmsendpoint";
...
export default function WorkPage() {
  ...
  useEffect(() => {
    async function fetchWorks() {
      setLoading(true);
      try {
        const data = await cmsFetch(CASE_STUDIES_QUERY);
        setWorks(data || []);
      } catch (err) {
        ...
        schema={
          works.length > 0
            ? {
                ...
                description:
                  "Explore case studies and projects by Alifia Hamzah, Product Designer.",
                ...
                {loading ? (
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-px bg-zinc-200 dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-800 rounded-2xl overflow-hidden shadow-sm">
                    {[...Array(ITEMS_PER_PAGE)].map((_, i) => (
                      <div
                        key={i}
                        className="group bg-white dark:bg-[#0A0A0B] flex flex-col justify-between h-full"
                      >
                        <SkeletonLoader className="w-full aspect-[16/10] overflow-hidden border-b border-zinc-150 dark:border-zinc-900/50" />
                        <div className="flex-1 p-6 flex flex-col justify-between space-y-4 text-left">
                          ...
                          <SkeletonLoader className="w-1/4 h-3 rounded-md" />
                        </div>
                      </div>
                    ))}
                  </div>
